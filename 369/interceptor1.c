...
// CHECK IF AX OR ORIG_AX
printk("interceptor");
if(check_pid_monitored(reg.ax, current->pid) == 1){
log_message(current->pid, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
//syscall(reg.ax,reg.bx,reg.cx,reg.dx,reg.si,reg.di,reg.bp);
...
if(current_uid() == 0 || current->pid == pid){
if(cmd == 3){
// spin_lock(&pidlist_lock);
add_pid_sysc(pid, syscall);
// spin_unlock(&pidlist_lock);
// add lock
while(pid == 0 && current_uid() == 0){
// spin_lock(&pidlist_lock);
...
if((cmd == 4 && check_pid_monitored(syscall, pid) == 0) || ((cmd == 2 || cmd == 3 || cmd == 4) && table[syscall].intercepted == 0 )){
return -EINVAL;
}
//took out cmd 1
if((cmd == 1 && table[syscall].intercepted == 1) || (cmd == 3 && check_pid_monitored(syscall, pid) == 1)){
return -EBUSY;
}
// End of error cases
...
// comment out?
table[__NR_exit_group].f = sys_call_table[MY_CUSTOM_SYSCALL];
table[__NR_exit_group].intercepted = 1;
table[__NR_exit_group].monitored = 0;
INIT_LIST_HEAD(&table[__NR_exit_group].my_list);
