...
void my_exit_group(int status)
{
	...
	// Remove the specific pid from the Syscall's list of monitored pids.
	del_pid(current->pid);
...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	// Check if pid is monitored for the specific syscall(reg.ax).
 ...
 *     c) Cannot start monitoring a pid for a system call that has not been intercepted.
 ...
 *
 * - Make sure to handle correctly the "Trick Case" described in the assignment handout.
 */
asmlinkage long my_syscall(int cmd, int syscall, int pid) {

	// Checks if the syscall is invalid.
	...
	if(syscall > 0 && syscall <= NR_syscalls){
		if(cmd == REQUEST_START_MONITORING || cmd == REQUEST_STOP_MONITORING){
			...
			if ((pid == 0 && current_uid() != 0) || (current_uid() != 0 && check_pid_from_list(current->pid, pid) != 0)) {
				return -EPERM;
			}
			// Checks if the requested pid is invalid.
			...
			/* Checks if a pid being requested to stop being monitored is not monitored or if a pid that has not been
			*intercepted is being requested to be monitored,to stop being monitored or to be released.*/
			if((cmd == REQUEST_STOP_MONITORING && check_pid_monitored(syscall, pid) == 0) || ((cmd == REQUEST_SYSCALL_RELEASE || cmd == REQUEST_START_MONITORING || cmd == REQUEST_STOP_MONITORING) && table[syscall].intercepted == 0 )){
					return -EINVAL;
			}
			// Checks if a pid being requested to be monitored is already monitored, return -EBUSY if it is.
...
//				return -EPERM;
//			}
			// Checks if process is root or if pid being asked to be monitored belongs to calling process.
			if(current_uid() == 0 || check_pid_from_list(current->pid, pid) == 0){

					// Lock pidlist and add pid to syscalls list of monitored pids then unlock.
					if(cmd == REQUEST_START_MONITORING){
						...
						// Add pid to syscalls list of monitored pids while pid is zero and process is root.
						if(pid == 0 && current_uid() == 0){
								spin_lock(&pidlist_lock);
								if (add_pid_sysc(current->pid, syscall) != 0) {
									return-ENOMEM;
								}
								spin_unlock(&pidlist_lock);
							}
			        	...
			        	//	 cmd is 4
							...
							// Lock pidlist, remove the specific pid from the syscalls list of monitored pids then unlock.
							spin_lock(&pidlist_lock);
		...
		// cmd 1/2
			...
			if(current_uid() == 0){
						if(cmd == REQUEST_SYSCALL_RELEASE){
								...
								// Lock pidlist and destroy the list of monitored pids for that specific syscall then unlock.
								spin_lock(&pidlist_lock);
								destroy_list(syscall);
								spin_unlock(&pidlist_lock);

								// comment out?
								// Lock calltable then make sys_call_table writable and return the original function.
								set_addr_rw((unsigned long)sys_call_table);
								spin_lock(&calltable_lock);
								sys_call_table[syscall] = table[syscall].f;

								// Unlock calltable and make sys_call_table read only.
...
static int init_function(void) {
	...
	// Initiaize table for each specific syscall.
	int i;
	...
	// Lock calltable and make sys_call_table writable.
	spin_lock(&calltable_lock);
	set_addr_rw((unsigned long)sys_call_table);

    // Put original custom syscall into orig_custom_syscall and replace original with my_syscall.
	...
	// Put original exit group into orig_exit_group and replace original with my_exit_group.
...
static void exit_function(void)
{
	   ...
	   // Return each syscall's function to its original and destroy its list of pids.
	   int i = 0;
	   for(i = 0; i < NR_syscalls; i++){
		...
		// Put original custom syscall back.
